...
func FormatIdr(price string) string {
	amount, _ := strconv.Atoi(price)
	p := message.NewPrinter(message.MatchLanguage("id")) // Gunakan "id" untuk bahasa Indonesia

	// Format angka dengan pemisah ribuan yang menggunakan titik dan pemisah desimal yang menggunakan koma
	formattedAmount := p.Sprintf("Rp. %v", amount)
	totalAmt := formattedAmount + ",00"
	return totalAmt
}
